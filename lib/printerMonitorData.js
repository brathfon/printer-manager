...
// get the complete history for the printer, not just the most recent
// returns a list of statuses

var getTonerStatusHistoryForPrinter = function(printerInfo, parserFunction) {
     var fullPath = path.join(ourPrinterMonitorDirectory, "printer_" + printerInfo.MACaddress);
     var filesInfo = getInformationForDirectory("printer_" + printerInfo.MACaddress);

     //console.log("In getTonerStatusHistoryForPrinter");
     //console.log(util.inspect(filesInfo, false, null));

     let statusList = [];
     for (let collectDay in filesInfo.filesInDirectory) {
        if (collectDay != "lastPingOKDate") {
          //console.log(`collectDay ${collectDay}`);
          if (filesInfo.filesInDirectory[collectDay].pingOK) {
            let tonerData = parserFunction(fullPath + "/" + collectDay);
            let tonerStatus = {};
            tonerStatus["parsingStatus"] = "successful";
            tonerStatus["room"] = printerInfo.room;
            tonerStatus["MACaddress"] = printerInfo.MACaddress;
            tonerStatus["IPaddress"] = printerInfo.IPaddress;
            tonerStatus["printerModel"] = printerInfo.printerModel;
            tonerStatus["statusDate"] = collectDay;
            tonerStatus["tonerStatus"] = tonerData;
            //console.log(`toner status for ${collectDay}`);
            //console.log(util.inspect(tonerStatus, false, null));
            statusList.push(tonerStatus);
          }
        }
     }
     //console.log(`toner statuses list`);
     //console.log(util.inspect(statusList, false, null));
...
// gets the most recent toner status for all printers in the printer info file
...
// read all the valid toner status to get the history, not just the most recent
var getTonerStatusHistory = function () {
 ...
 readPrinterInfoFile().forEach( function (printerInfo) {
     ...
     switch (printerInfo.printerModel) {
       case 'Ricoh-SP3400N':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusRicoh3400));
         break;
       case 'Ricoh-SP3500N':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusRicoh3500));
         break;
       case 'Ricoh-SP3600N':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusRicoh3600));
         break;
       case 'Ricoh-SPC242SF':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusRicoh242));
         break;
       case 'Ricoh-SPC252SF':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusRicoh252));
         break;
       case 'HP-LaserJet-Pro-P1606dn':
         tonerStatuses = tonerStatuses.concat(getTonerStatusHistoryForPrinter(printerInfo, printerMonitorParser.getTonerStatusHP_P1606dn));
         break;
       default:
         console.log("Do not know how to parse the file for a : " + printerInfo.printerModel);
         //tonerStatuses = tonerStatuses.concat(getTonerStatusForNonParsingPrinter(printerInfo));
     }
   });
...
exports.getTonerStatusHistory = getTonerStatusHistory
