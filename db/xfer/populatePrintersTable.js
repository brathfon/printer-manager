...
var getModels = function (data, endConnection, callback) {
  ...
  connection.query("select model_id, model_name, is_color, data_collection_method from models", function(err, rows, fields) {
    ...
    if (!err) {
      ...
      data['modelNameToDataCollectionMethod'] = {};
      for (i in rows) {
        data['modelNameToModelId'][rows[i].model_name] = rows[i].model_id;
        data['modelNameToDataCollectionMethod'][rows[i].model_name] = rows[i].data_collection_method;
      }
      if (callback) {
...
var printLookupData = function (data, endConnection, callback) {
  ...
  for (let model_name in data.modelNameToDataCollectionMethod) {
    console.log("-- model_name " + model_name + " can_snmp_monitor " + data.modelNameToDataCollectionMethod[model_name]);
  }
...
var readPrinterTsv = function (data, endConnection, callback) {
  ...
  lines.forEach( function (line) {
    ...
    let is_monitored = 0;
    if (data.modelNameToDataCollectionMethod[model] === "snmp") {
      is_monitored = 1;
    }
    ...
    // form a printer name
    let is_active = 1;  // these should all be active
    let insertStatement = `INSERT INTO printers (mac_address, ip_address, model_id, tag_number, room, is_monitored, is_active)`;
    insertStatement += ` VALUES ("${mac_address}", "${ip_address}", ${model_id}, ${tag_number}, "${room}", ${is_monitored}, ${is_active});`;
